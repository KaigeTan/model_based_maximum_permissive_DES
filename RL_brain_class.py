    ...
    def choose_action_Train(self, S, param, IfAssignInit):
        ...
        if(np.random.uniform() > self.epsilon):
            ...
            if self.look_ahead_step == 2:
                pattern_value, pattern_length = self.Q_value_eval(S, param)
        ...
        while(len(reach_states) != 0):
            ...
            if not any(sub_S == sub_s[0:-3] + [sub_s[-1]] for sub_s in generated_states):
                ...
                if self.look_ahead_step == 2:
                    ...
                    # perform rollout, run monte-carlo traces for 50 times, depth is 4.
                    for i in range(self.RO_traces):
                        ...
                        for _ in range(self.RO_depth):
                            ...
                            if isDone_test: # TODO: confirm which way is better to handle a MC trace blocking
                                ...
                                rollout_method = 2 # 1 for assign negative val, 2 for cut the path
                                if rollout_method == 1:
                ...
                #     pattern_value = self.sess.run(self.q_eval, feed_dict = {self.S: S_norm})
                #     sorted_indices = np.argsort(-pattern_value)
                #     top_N_indices = sorted_indices[0][:MC_nodes]
                #     pattern_value = np.zeros(self.n_actions)
                #     for pattern_ind in top_N_indices:
                ...
                #             Q_s1 = []
                #             for S_ in all_S_:
                #                 pattern_value_s2, pattern_length = self.Q_value_eval(S_, param)
                #                 # TODO: if at least two patterns can lead to deadlock in s_t+2, then this is a deadlock state
                #                 if np.count_nonzero(pattern_value_s2 == -1e5) >= 2:
                #                     Q_s1.append(-1e5) # indicate a deadlock happens in s_t+2
                #                 else:
                #                     Q_s1.append(np.max(pattern_value_s2)) # the expected value of S_t+1, based on onestep lookahead
                #             Q_s1_exp = sum(Q_s1)/len(Q_s1) # mean, since it has possiblity of stepping into any states
